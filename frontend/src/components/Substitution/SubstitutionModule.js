...
const SubstitutionModule = () => {
  ...
  const fetchApprovedLeaves = async () => {
    ...
    // Only show approved leaves that need substitution
  ...
  const handleSuggest = async () => {
    ...
    setSuggestions([]);
    if (!selectedLeave) {
      setMessage('❌ Please select a leave request first!');
      return;
    }
    ...
    try {
      ...
      if (res.data.suggestions.length === 0) {
        setMessage('❌ No free faculty available at this slot!');
      }
    } catch (err) {
          ...
          >
            ...
            {leaves.map(l => (
              <option key={l._id} value={l._id}>
                {l.facultyId?.name || 'Faculty'} | {l.day} | {l.timeSlot} | {l.courseName}
              </option>
            ))}
          </select>
              ...
              <div className="suggest-card" key={s.faculty._id}>
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                  <div>
                    ...
                    <p>Current Workload: {s.totalHours} hrs/week</p>
                  </div>
                  <button className="btn-success" onClick={() => handleAssign(s)}>
                    Assign ✓
                  </button>
                </div>
              </div>
