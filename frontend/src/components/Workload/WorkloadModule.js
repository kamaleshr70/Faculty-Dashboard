...
const WorkloadModule = () => {
  ...
  useEffect(() => { fetchFaculties(); fetchWorkloads(); }, []);
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      setMessage('✅ Workload added!');
      setFacultyId(''); setCourseName('');
      setDay(''); setTimeSlot(''); setHoursPerWeek('');
      fetchWorkloads();
    } catch (err) {
  ...
  return (
    <div>
      ...
      {/* Both see Workload Table */}
      <div className="card">
        <h3>📚 All Workload Assignments</h3>
        <table>
          <thead>
            <tr>
              <th>#</th><th>Faculty</th><th>Course</th>
              <th>Day</th><th>Time Slot</th><th>Hrs/Week</th>
